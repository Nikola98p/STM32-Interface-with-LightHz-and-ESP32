...
uint8_t Is_First_Captured = 0; // da li je prva vrednost snimljena? 0 - ne; 1 - da
	 ...
	 /* void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
	  	{
	  		if (htim -> Channel == HAL_TIM_ACTIVE_CHANNEL_1)
	  		{
	  			if(Is_First_Captured == 0)
	  			{
	  				IC_Value1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1); //snimi prvu vrednost
	  				Is_First_Captured = 1; // zapamti da je snimljena
	  			}
	  			else if (Is_First_Captured == 1) // proveri da li je snimljena?
	  			{
	  				IC_Value2 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1); //snimi drugu
	  				if (IC_Value2 > IC_Value1)
	  				{
	  					Difference = IC_Value2-IC_Value1; //zracunaj razliku
	  				}
	  				else if (IC_Value2 < IC_Value1)
	  				{
	  					Difference = ((0xffff-IC_Value1)+IC_Value2)+1;
...
/*void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	if(htim == &htim2){
		HAL_UART_Transmit(&huart2, 48, 1, 1);
	}
}*/
